...
//    to navigate to an input with the next/previous buttons thats outside a modal.
function preventScrollMobileSafari() {
  ...
  const onTouchMove = (e: TouchEvent) => {
    ...
    // the top but its the best we can do.
  ...
  const onTouchEnd = (e: TouchEvent) => {
    ...
    // Apply this change if were not already focused on the target element
  ...
  const onFocus = (e: FocusEvent) => {
    ...
    if (isInput(target)) {
      ...
      requestAnimationFrame(() => {
        ...
        // so we need to do this ourselves in a way that doesnt cause the whole page to scroll.
        if (visualViewport) {
